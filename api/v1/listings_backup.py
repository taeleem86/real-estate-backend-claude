    ...
    try:
        ...
        for prop_dict in listings:
            try:
                ...
                # 매물을 네이버 형식으로 변환
                # Kept property_id for get_naver_compatible_response as per
                # instruction
                ...
                if naver_response:
                    ...
                    # 응답 데이터 구성
                    naver_listing_item = NaverPropertyResponse(
                        ...
                        naver_format=NaverPropertyData(**naver_response.get("naver_format", {})),
                        naver_compatibility=NaverValidation(**naver_response.get("naver_compatibility", {})) if include_validation else None
                    )
        ...
        # 응답 데이터 구성
        response_data = NaverPropertyListResponse(
            data=naver_listings,
...
@router.get("/{listing_id}/naver-format", response_model=NaverPropertyResponse)
async def get_listing_naver_format(
    ...
    try:
        ...
        # 네이버 호환 형식으로 변환
        # Kept property_id for get_naver_compatible_response as per instruction
        naver_response = await service.get_naver_compatible_response(
            listing_id,
            include_naver=True
        )
        ...
        # 응답 데이터 구성
        try:
            naver_format_data = naver_response.get("naver_format", {})
            response_data = NaverPropertyResponse(
                id=str(listing_id),  # Use listing_id
                title=naver_response.get("title", ""),
                property_type=naver_response.get("property_type", ""),
                deal_type=naver_response.get("deal_type", ""),
                price=str(naver_response.get("price", "")),
                ...
                created_at=naver_response.get("created_at"),
                updated_at=naver_response.get("updated_at"),
                naver_format=NaverPropertyData(**naver_format_data),
                naver_compatibility=NaverValidation(
                    **naver_response.get("naver_compatibility", {})) if include_validation else None
            )
        except Exception as model_error:
            # Pydantic 오류 발생 시 JSON 응답으로 처리
            return JSONResponse(
                status_code=status.HTTP_200_OK,
                content={
                    "id": str(listing_id),
                    ...
                    "debug_error": f"Pydantic 모델 오류: {str(model_error)}"
                }
            )
